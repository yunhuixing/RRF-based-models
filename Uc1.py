...
is_cal = mp.notna() & mp.astype(str).str.strip().ne("") & mp.astype(str).str.lower().ne("nan")
...
print(f"StatCal model fitted: RRF = {a:.3e} + {b:.3e}·X2")

df["Pred_RRF"] = model.predict(df["X2"].values.reshape(-1, 1))
...
out_xlsx = OUT_DIR / "StatCal_a_plus_bX2_results.xlsx"
...
if ENABLE_PLOT:
    ...
    RRF_line = model.predict(X2_line)
    ...
    plt.tight_layout()
    fig_path = OUT_DIR / "StatCal_a_plus_bX2_2D.png"
