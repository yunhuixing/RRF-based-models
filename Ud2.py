...
for _, row in test_points.iterrows():
    ...
    X2 = subset["X2"].values.reshape(-1, 1)
    X_quad = np.hstack([X2, X2**2])
    y = subset["RRF"].values
    ...
    X2_all = df.loc[mask, "X2"].values.reshape(-1, 1)
    X_all_quad = np.hstack([X2_all, X2_all**2])

    df.loc[mask, "Pred_RRF"] = model.predict(X_all_quad)
...
out_xlsx = OUT_DIR / "DynCal_a_plus_bX2_plus_cX2sq_results.xlsx"
...
if ENABLE_PLOT:
    ...
    plt.tight_layout()
    fig_path = OUT_DIR / "DynCal_a_plus_bX2_plus_cX2sq_2D.png"
