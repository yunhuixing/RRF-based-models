...
SHEET_NAME = "oxc"
OUT_XLSX = os.path.join(OUT_DIR, "oxc_all_results.xlsx")
...
def main():
    ...
    for _df in (train_df, test_df):
        _df["ln_X1"] = np.log(_df["X1"])
        _df["ln_X2"] = np.log(_df["X2"])
        _df["ln_RRF"] = np.log(_df["RRF"])

    X = sm.add_constant(train_df[["ln_X1", "ln_X2"]])
    y = train_df["ln_RRF"]
    model = sm.OLS(y, X).fit()

    for _df in (train_df, test_df):
        Xp = sm.add_constant(_df[["ln_X1", "ln_X2"]])
    ...
    Xg = pd.DataFrame({
        "ln_X1": np.log(X1g.ravel()),
        "ln_X2": np.log(X2g.ravel())
    })
    Xg = sm.add_constant(Xg)
    Zg = np.exp(model.predict(Xg)).values.reshape(X1g.shape)
